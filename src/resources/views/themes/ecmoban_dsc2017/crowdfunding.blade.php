...
@endif
 class="p-bar-green"></div>
                    ...
                    <p class="p-target" id="projectMessage">{{ $lang['project_Prompt_one'] }} <span class="f_red">{{ $zhongchou['zw_end_time'] }} </span>{{ $lang['project_Prompt_two'] }} <span class="f_red"><i>￥</i>{{ $zhongchou['amount'] }}</span>{{ $lang['project_Prompt_three'] }}
...
@if($shop_information['is_dsc'])
                                    ...
                                    <a id="IM" onclick="openWin(this)" href="javascript:;" goods_id="{{ $goods['goods_id'] }}" class="seller-btn"><i class="icon"></i>{{ $lang['Contact_us'] }}</a>
...
@if($item['shenyu_ren'] > '0'||$item['limit'] == '-1')
									...
									<button type="button" class="common-btn" onclick="zc_goods(this)" gid="{{ $item['id'] }}">{{ $lang['Support'] }}￥{{ $item['price'] }}</button>

@else

									<button type="button" class="common-btn-unuse" onclick="" gid="{{ $item['id'] }}">{{ $lang['Support'] }}￥{{ $item['price'] }}</button>
