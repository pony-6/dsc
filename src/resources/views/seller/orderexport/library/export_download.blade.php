...
<script type="text/javascript">
	...
	function start(page_down)
	{
        ...
        $.post("{{ route('seller/ajax_export_download') }}", args, function (result) {
	...
	function order_download_list_csv(result)
	{
        ...
        $.post("{{ route('seller/export_download_list_csv') }}", args, function (result) {
	...
	function start_response_csv(result){
		if(result.is_stop == 0){
            ...
            // 下载压缩文件
            location.href = "{{ route('seller/export_download_list_csv', ['act' => 'download_zip']) }}";
		}
	}
