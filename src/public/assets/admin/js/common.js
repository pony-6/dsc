        ...
        if(total != 0){
			//订单提醒 start
				...
				if(await_ship>=0 && await_ship<100){
					$("*[ectype='orderMsg']").html("").append('<p><a href="javascript:void(0);" data-url="order.php?act=list&serch_type=8" data-param="menushopping|02_order_list" target="workspace" class="message" >待发货订单</a> <span class="tiptool">（<em id="no_paid">'+await_ship+'</em>）</span></p>')
				}else if(await_ship>99){
					$("*[ectype='orderMsg']").html("").append('<p><a href="javascript:void(0);" data-url="order.php?act=list&serch_type=8" data-param="menushopping|02_order_list" target="workspace" class="message" >待发货订单</a><span class="tiptool">（<em id="no_paid">99+</em>）</span></p>')
				}
				//待发货订单 end
			...
			//会员提醒 start
				...
				if(user_discuss >= 0  && user_discuss<100){
					$("*[ectype='userMsg']").html("").append('<p><a href="javascript:void(0);" data-url="discuss_circle.php?act=list&review_status=1" target="workspace" class="message">网友讨论圈审核</a> <span class="tiptool">（<em id="user_discuss">'+user_discuss+'</em>）</span></p>')
				}else if(user_discuss > 99){
					$("*[ectype='userMsg']").html("").append('<p><a href="javascript:void(0);" data-url="discuss_circle.php?act=list&review_status=1" target="workspace" class="message">网友讨论圈审核</a><span class="tiptool">（<em id="user_discuss">99+</em>）</span></p>')
				}
				//网友讨论圈审核 end
			//会员提醒 end
...
function set_chart_view(chart_date, chart_id){
    ...
    if(chart_date.total_money){
    	$("[ectype='total_money']").text(Math.round(chart_date.total_money*100)/100);
    }
}
...
//删除图片 type：图片类型；cat_id：分类id；
function delCateImg(type, cat_id)
{
	var t = confirm(jl_del_cate_img );
	if(t)
	{
		$.ajax({
			type:'get',
			url:'category.php',
			data:'act=delete_icon_remove&type='+type+'&cat_id='+cat_id,
			dataType:'json',
			success:function(data)
			{
				window.location.reload();
