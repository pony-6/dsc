...
$(function () {
    ...
    $("#face").click(function (e) {
        ...
        layui.use(['layer'], function () {
            ...
            var isShow = $(".layui-chat-face").css('display');
            if ('block' == isShow) {
                layer.close(index);
                return;
            }
...
function showFaces() {
    ...
    var _html = '<div class="layui-chat-face"><ul class="layui-clear chat-face-list">';
...
function msgFactory(content, type, uinfo) {
    var _html = '';
    if ('mine' == type) {
        _html += '<li class="chat-chat-mine">';
    } else {
        _html += '<li>';
    }
    _html += '<div class="chat-chat-user">';
    ...
    _html += '</div><div class="chat-chat-text">' + replaceContent(content) + '</div>';
    _html += '</li>';
    ...
    $.getJSON('/service/index/getChatLog', {uid: uid, page: page}, function(res){
        if(1 == res.code && res.data.length > 0){
            ...
            for(var i = 0; i < len; i++){
                ...
                _html += '</div><div class="chat-chat-text">' + replaceContent(v.content) + '</div>';
                _html += '</li>';
            }
...
function showBigPic(){
    ...
    $(".layui-chat-photos").on('click', function () {
        var src = this.src;
        layer.photos({
            photos: {
                data: [{
